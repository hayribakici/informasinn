...
class InformationRetriever {
  ...
  final _models = <String, String>{
    ...
    'huihui-ai': 'huihui-ai/Qwen2.5-14B-Instruct-abliterated-v2'
  };
  ...
  Stream<String> retrieveAsStream(String prompt) {
    return client
        .streamCompletion(
      messages: LlmMessage.basic(prompt),
  ...
  Future<String> getInformationFromData(String prompt) async {
    try {
      var response = await client.getCompletion(
        providersOrder: [LlmProviders.featherless],
        messages: LlmMessage.basic(prompt),
        modelId: defaultModel,
      );
      return response.choices.first.content;
    } on Exception catch (e, s) {
      print("API Error during stream: $e $s");
      return "error astf";
    }
  }

  List<LlmMessage> _createMessage(String prompt) => LlmMessage.basic(prompt);
}
